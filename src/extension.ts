...
export function activate(context: vscode.ExtensionContext) {
	let disposable = vscode.commands.registerCommand('make-mui-style.makeMeStyle', async () => {
		...
		const fileNameExt = fileNameSplitted[fileNameSplitted.length -1 ];
		...
		const styleNameFile = replaceLastOccurrenceInString(`${capitalice(fileNameExt.replace(ext, ""))}Style`, ".", "");
		...
		try {
			...
			const doc = await vscode.window.showTextDocument(textDocument, 1, false)
			doc.edit(e => {
				e.insert(new vscode.Position(0, 0), template(capitalice(styleNameFile)))
			})
